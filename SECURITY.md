...
The Bakobo corporate site and the checks that publish it. In scope: anything that would let content be published here that Bakobo did not write, and anything in the published `.well-known` material that misstates a Bakobo identifier — the OOBIs in particular, since a wrong one points somebody at the wrong witness for a Bakobo AID.

This site also serves the RFC 9116 [`/.well-known/security.txt`](https://bakobo.com/.well-known/security.txt), which is the canonical machine-readable pointer for anything Bakobo runs, not just this repository. The two agree by construction: both name `security@bakobo.com`, and a test in this repo fails thirty days before that file's `Expires` date so the promise cannot quietly go stale.
